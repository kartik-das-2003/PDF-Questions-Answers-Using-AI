...
async function uploadFiles(files) {
  ...
  if (!pdfs.length) {
    setStatus("Please upload PDF files only.", "error");
    return;
  }
  ...
  for (const file of pdfs) {
    ...
    try {
      ...
      setStatus(`✓ ${file.name} — ${data.chunks} chunks indexed`, "success");
      await loadDocuments();
    } catch (error) {
  ...
  chat.innerHTML = `
    <div id="welcome" class="welcome-state">
      ...
      <h2>Turn pages into answers.</h2>
        <p>Upload one or more PDFs, then ask questions in plain English. MayurQA finds relevant passages and shows you exactly where every answer came from.</p>      <div class="suggestions" aria-label="Suggested questions">
