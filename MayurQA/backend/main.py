...
@app.get("/")
def root():
    return {"status": "MayurQA backend running"}
...
async def upload_pdf(files: list[UploadFile] = File(...)):
    ...
    for file in files:
        if not file.filename.endswith(".pdf"):
            results.append({"filename": file.filename, "success": False, "error": "Only PDF files are supported."})
            continue
        doc_id = str(uuid.uuid4())
        save_path = os.path.join(UPLOAD_DIR, f"{doc_id}.pdf")
        with open(save_path, "wb") as f:
        ...
        try:
            chunk_count = process_pdf(save_path, doc_id, file.filename)
            results.append({"doc_id": doc_id, "filename": file.filename, "chunks": chunk_count, "success": True})
        except Exception as e:
            os.remove(save_path)
